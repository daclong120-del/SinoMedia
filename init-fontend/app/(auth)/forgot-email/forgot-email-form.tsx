...
const ENGLISH_DICT = {
  title: "Forgot email",
  description: "Enter a domain name on your account to find the associated email address.",
  ...
  domainInvalid: "Please enter a valid domain name (e.g., example.com)",
  successMsg: "If a matching account exists for that domain, we have sent recovery instructions to its email address.",
};

const DICT: Record<string, typeof ENGLISH_DICT> = {
  "English (US)": ENGLISH_DICT,
  "Tiếng Việt": {
    title: "Quên địa chỉ email",
    description: "Nhập tên miền trong tài khoản của bạn để tìm địa chỉ email liên kết tương ứng.",
    ...
    domainInvalid: "Địa chỉ tên miền không hợp lệ (ví dụ: example.com)",
    successMsg: "Nếu tài khoản khớp với tên miền đó tồn tại, chúng tôi đã gửi hướng dẫn khôi phục tới địa chỉ email của tài khoản.",
  }
};
...
export default function ForgotEmailForm() {
  ...
  useEffect(() => {
    document.title = (lang === "Tiếng Việt" ? "Quên địa chỉ email" : "Forgot email") + " | Cloudflare";
  }, [lang]);
  ...
  const validateDomain = (val: string) => {
    if (!val.trim()) {
      return d.domainRequired;
    }
    const domainRegex = /^([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,}$/i;
    if (!domainRegex.test(val.trim())) {
      return d.domainInvalid;
    }
    return "";
  };
      ...
      {isSuccess ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className={`w-full px-3 py-2 rounded-lg border bg-white dark:bg-zinc-800 text-sm font-medium transition-all outline-none focus:ring-1 focus:ring-primary ${
                domainError ? "border-destructive focus:ring-destructive" : "border-neutral-300 dark:border-zinc-700 hover:border-neutral-400 focus:border-primary"
              }`}
              disabled={isLoading}
              autoCapitalize="none"
              autoComplete="off"
          ...
          <button
            ...
            disabled={isLoading || !domain || validateDomain(domain) !== ""}
          >
            {isLoading ? (
              <>
