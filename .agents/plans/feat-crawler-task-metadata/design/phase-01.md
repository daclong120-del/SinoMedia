# 📐 DESIGN — Phase 1: Database Migration & Schema Update

Tài liệu thiết kế chi tiết các thay đổi đối với cơ sở dữ liệu Supabase trong Phase 1.

---

## 1. Bản vẽ DDL Bảng biểu

### A. Bảng `crawler_tasks`
Thêm cột `metadata` để lưu cấu hình JSON động nhận từ Dashboard.
...
### B. Bảng `crawled_posts`
Bổ sung hai cột để ghi nhận trực tiếp Tags và Ngôn ngữ từ nhiệm vụ cào, giúp tối ưu hóa hiệu năng câu truy vấn tìm kiếm/lọc trên Creative Hub.
...
## 2. Hàm RPC `create_crawler_tasks`
Hàm PL/pgSQL này được cập nhật để trích xuất trường `metadata` từ đối tượng JSON đầu vào (`p_tasks`) và đưa vào câu lệnh chèn (`INSERT`).
...
DECLARE
  ...
  v_max_count int;
  v_priority text;
  v_metadata jsonb; -- Biến lưu trữ metadata
  v_result jsonb;
BEGIN
  -- Kiểm tra giới hạn tối đa 50 tasks
  ...
  FOR v_task IN SELECT * FROM jsonb_array_elements(p_tasks) LOOP
    ...
    v_metadata := coalesce(v_task->'metadata', '{}'::jsonb); -- Trích xuất metadata
    ...
    ELSE
      -- Thêm cột metadata vào câu INSERT
