...
export function redactSecrets(val: any): any {
  ...
  if (typeof val === "string") {
    ...
    // Mask msToken / xsec_token / api_token / xsecToken / xmst
    redacted = redacted.replace(/(msToken|xsec_token|api_token|token|xsecToken|xmst)[=:\s'"`]+([a-zA-Z0-9_\-\.]+)/gi, (match, p1, p2) => {
    ...
    // 1. Trường hợp có dấu nháy bao quanh (ví dụ trong JSON hoặc string): cookie: "a=1; b=2"
    redacted = redacted.replace(/(cookie_data|cookies|cookie)([=:\s]+)(["'])([^"'\n\r]+)(["'])/gi, '$1$2$3[MASKED]$5');
    // 2. Trường hợp không có dấu nháy bao quanh: cookie=a=1; b=2
    redacted = redacted.replace(/(?<!["'])(cookie_data|cookies|cookie)([=:\s]+)([^"'\n\r\s,{}()[\]]+)/gi, '$1$2[MASKED]');

    // Mask password
    redacted = redacted.replace(/(password|pass)[=:\s'"`]+([^\s;\n\r"'}]+)/gi, (match, p1, p2) => {
      return `${p1}=***`;
    });
    ...
    // Generic Token Redactor: Mask bất kỳ chuỗi alphanumeric/base64 liên tục dài >= 24 ký tự (JWTs, API keys, hashes...)
    // Loại trừ URLs, paths và email
    redacted = redacted.replace(/(?<![a-zA-Z0-9\/\\@])([a-zA-Z0-9_\-\.]{24,})(?![a-zA-Z0-9])/g, (match) => {
