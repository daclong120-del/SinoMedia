...
async function persistPost(videoItem: any, authorUuid?: string): Promise<string> {
  ...
  if (postRow.media_urls && postRow.media_urls.length > 0) {
    for (let i = 0; i < postRow.media_urls.length; i++) {
      ...
      try {
        ...
        if (exists) {
          r2Urls.push(`kuaishou/${postRow.platform_id}/${filename}`);
        } else {
        ...
        console.log(`[KuaishouCrawler] Không thể tải file đính kèm lên R2 cho bài viết ${postRow.platform_id}:`, e);
  ...
  if (postRow.cover_url) {
    try {
      ...
      let coverUrlR2 = "";
      if (exists) {
        coverUrlR2 = `kuaishou/${postRow.platform_id}/${filename}`;
      } else {
        const buf = await downloadKuaishouMedia(postRow.cover_url);
        coverUrlR2 = await uploadMediaToR2("kuaishou", postRow.platform_id, filename, buf, "image/jpeg");
      }
      postRow.cover_url = coverUrlR2;
    } catch (e) {
      console.log(`[KuaishouCrawler] Không thể tải cover lên R2 cho bài viết ${postRow.platform_id}:`, e);
    }
  }
