...
> **Cập nhật 2026-07-13:** Douyin là ngoại lệ vận hành của mô hình cookie-only. Raw cookie không đủ để tạo session ổn định vì Douyin gắn cookie với browser context/localStorage/fingerprint. Hướng hiện tại là dùng **Playwright Chromium persistent context** như một bước **session bootstrap/hydrator** (`raw cookie -> browser context -> enriched DouyinSession -> diagnostic -> HTTP API crawler`). Browser bootstrap không thay thế HTTP API crawler và không được dùng làm crawler runtime mặc định.
...
|---|---|---|
| **Douyin** | Crawler chính vẫn là HTTP API (`impit`/`fetch` + `signDetail()` khi cần), nhưng session phải đi qua diagnostic hard gate. Raw cookie cần được hydrate bằng Playwright Chromium persistent context để xuất `DouyinSession` đầy đủ nếu thiếu `msToken`, `webid`, `fp`, `uifid`. | Không dùng browser để crawl mặc định. Browser chỉ là bootstrap/hydrator. Nếu enriched session không pass diagnostic -> fail fast. |
...
## 2026-07-21 Addendum - Douyin challenge recovery

Douyin `verify_check` must be handled as session recovery, not as a return to full browser crawling.

- Keep Douyin search/detail/comment HTTP-first after session passes diagnostic.
- Use Playwright persistent context only for bootstrap/recovery.
- Use 2Captcha only through a bounded worker-side challenge provider when configured.
- Store/display solver settings in Dashboard, but run solver only in `crawler-pipeline`.

Detailed source of truth: `docs/architecture/douyin-session-recovery.md`.
