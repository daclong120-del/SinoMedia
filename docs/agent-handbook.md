...
- **Warm-up Cookie & Zhihu Search**: API search Zhihu yêu cầu cookie được đồng bộ đầy đủ từ trang chủ và phiên search thật. Bắt buộc phải khởi chạy Playwright mở trang search thật (`/search?q=...`) trong 5 giây để warm-up/sync cookie trước khi gửi API request.
