...
- **Quyết định:**
  1. Thiết lập hàm `validateCrawlerAccountGet(searchParams)` phân tách rõ rệt 2 Mode cho GET `crawler_accounts`:
     - *Mode 1 (Checkout)*: Bắt buộc truyền platform active với limit=1. Trả về và cưỡng chế chỉ select `id`, `username`, `cookie_data`.
     - *Mode 2 (Status check)*: Chỉ cho phép query theo ID (`id=eq.<uuid>`). Cưỡng chế chỉ select `id`, `status`, `failure_count` và **tuyệt đối không trả về `cookie_data`**.
...
- **Đánh đổi:** Chấp nhận giảm tính linh hoạt khi truy vấn thông tin accounts qua REST Proxy từ phía Worker.
...
- **Bối cảnh:** Cần xác định rõ ranh giới và thứ tự ưu tiên cho việc phát triển Desktop App để tránh sa đà vào thiết kế UI native hay bộ installer quá sớm khi nhân chạy độc lập (portable runtime) chưa ổn định.
- **Quyết định:**
